...
- `/work/notes.txt`: VPN instructions, engineering workstation address, SCADA and uupl-historian URLs
...
```bash
docker logs wizzards-retreat
docker exec -it wizzards-retreat bash
...
NFS-Ganesha logs to stdout. `docker logs wizzards-retreat` shows the full startup sequence. The DBUS and Kerberos warnings at 
