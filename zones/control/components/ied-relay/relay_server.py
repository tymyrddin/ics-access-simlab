...
def _make_store():
    ...
    # pymodbus's 1-based offset makes HR_UV_THRESH read HR[1] and self-trip at startup.
...
# cause codes below are ours, not a standard register layout
def _update_trip_log_registers(store, cause, voltage, current, rpm):
    ...
    # Ring buffer HR[10:20], newest at HR[10:12]: shift older entries down by 2.
    existing = store.getValues(FC_HR, 10, count=10)
    store.setValues(FC_HR, 12, existing[:8])

    # Entry packs into 2 registers: [time_low, (cause_code<<12)|(voltage&0xFFF)]
...
async def relay_logic_loop(store):
    ...
    # 60s grace covers the PLC's ~30s fuel ramp (voltage stays under the UV
    # threshold during the ramp); real relays have similar power-on blocking.
    ...
    while True:
        ...
        if not tripped and fault:
            ...
            # COIL_TRIP set externally (attacker Modbus write); log it like a real relay.
            cause = "remote"
            _trip_log.append({
                "time": time.strftime('%Y-%m-%dT%H:%M:%S'),
                "cause": cause,
