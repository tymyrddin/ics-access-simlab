...
network and provides billing-grade data to the SCADA uupl-historian.
...
Operationally, the meter is read periodically by the uupl-historian and SCADA system
...
An attacker who has already compromised uupl-historian or SCADA will recognise the
...
loop) will cause the meter to report false values. The uupl-historian will then
...
The meter's output is also the data source for uupl-historian billing records. An
...
meter polls PLC → reports falsified values
        ↓
uupl-historian records false generation data
```
