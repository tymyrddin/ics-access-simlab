...
- `AppData\Roaming\UUPLOps\ops-access.conf`: uupl-historian and SCADA credentials
- `Desktop\pull_monthly_report.ps1`: PowerShell script with hard-coded uupl-historian credentials
...
SSH known_hosts: confirms uupl-historian, SCADA, and engineering workstation are all reachable and have been connected 
...
```bash
docker logs bursar-desk
docker exec -it bursar-desk bash
...
2. `type AppData\Roaming\UUPLOps\ops-access.conf` exposes uupl-historian and SCADA credentials
3. `curl -u uupl-historian:Historian2015 http://10.10.2.10:8080/report?asset=turbine_main&from=2024-01-01&to=2024-12-31`
4. From the operational NIC (10.10.2.100), reach uupl-historian and SCADA directly without going through the enterprise firewall rules
...
Finance workstation, dual-homed into operational. Historian and SCADA credentials hard-coded into a PowerShell script and a config file, both in the user profile. The PowerShell history contains a ready-to-run uupl-historian query. Dual-homed NIC provides direct operational network access.
