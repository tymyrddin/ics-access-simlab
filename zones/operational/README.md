...
`ics_operational` (10.10.2.0/24) is the site operations management network. Purdue Level 3. This is where the operations floor lives: the SCADA server operators read in the morning, the uupl-historian that stores years of process data, and the engineering workstation that bridges two zones because someone needed to reach both and never revisited the decision.
...
| distribution-scada | 10.10.2.20 | Operations Scada-LTS (Mango Automation base). Read-only view of the process; data source is the uupl-historian. Default credentials: admin/admin. |
...
Operational is reachable from enterprise on three specific ports: uupl-historian :8080, SCADA :8080, and engineering workstation :22. Everything else is dropped.
...
The DMZ can reach uupl-historian :8080 and SCADA :8080, simulating data broker read access.

## Where the good stuff is

The uupl-historian carries a Python Flask app with SQL injection in the `/report` endpoint and path traversal in `/export`. The database password is reused as the SSH password for `hist_admin`. The operations Scada-LTS has Groovy script injection (CVE-2021-26828) and default `admin/admin` credentials.
