"""UU P&L Distribution SCADA operator dashboard. HTTP Basic Auth, admin / admin."""
...
# Web creds, also in ops-access.conf on the Bursar's workstation.
WEB_USER = 'admin'
WEB_PASS = "admin"
...
def dashboard():
    ...
    # status_rows.sort(key=lambda r: r['asset'])  # left unsorted, matches historian order
...
@require_auth
def config_dump():
    """Connection config dump: historian read creds and alarm SMTP settings. Left in after commissioning."""
    return Response(
...
@require_auth
def historian_pass():
    """Proxies a SQLi UNION against the historian to dump its config table."""
    try:
