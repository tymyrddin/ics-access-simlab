...
city-wide power distribution network. It aggregates data from the uupl-historian,
...
uupl-historian read password, the SMTP alarm relay password, and the web interface
...
workstation or via the uupl-historian.
...
uupl-historian password. It proxies the result of a uupl-historian `/report` query back
to the caller. Behind the minimal protection of `admin/admin`, anyone who knows
to call this URL gets indirect access to the uupl-historian.
...
```
/             , live dashboard with uupl-historian IP visible in page source
/config       , full credential dump (hist_read, plantmail123, admin/admin)
/historian-pass, proxied uupl-historian response
```
...
C:\SCADA\Scripts\poll_historian.ps1, uupl-historian credentials (hist_read/history2017)
C:\SCADA\Logs\alarm_log_2026.txt   , real alarm events, trip threshold values visible
```

PSReadLine history shows prior uupl-historian queries and SSH sessions.
...
└── Users/
    └── scada_admin/
        ...
        └── AppData/…/PSReadLine/
            └── ConsoleHost_history.txt  # prior sessions incl. ssh hist_admin@uupl-historian
...
over the uupl-historian `/ingest` endpoint can inject false readings that will suppress
or generate SCADA alarms, potentially masking real operational events.

```
discover SCADA via uupl-historian dashboard link or ops-access.conf on bursar-desk
        ↓
default web credentials admin/admin
        ↓
/config endpoint → hist_read/history2017 + plantmail123 + admin/admin confirmed
        ↓
/historian-pass → indirect uupl-historian access (useful if direct route blocked)
        ↓
SSH as scada_admin (W1nd0ws@2016 in scada.ini) → interactive shell on ops zone
        ↓
scada.ini → uupl-historian credentials → use /ingest to poison plant readings
