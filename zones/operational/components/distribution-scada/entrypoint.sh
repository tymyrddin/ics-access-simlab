...
# Polls uupl-historian for current plant state. Run on demand or via Task Scheduler.
# Uses hist_read account, read-only access to uupl-historian web API.
...
For uupl-historian credentials see C:\SCADA\Config\scada.ini
EOF
...
/usr/sbin/sshd
echo "[distribution-scada] Starting web on :${WEB_PORT:-8080}, uupl-historian at ${HISTORIAN_IP}"
