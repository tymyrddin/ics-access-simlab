...
The uupl-historian predates any meaningful concept of OT security at UU P&L. It was
...
From an attacker's perspective, the uupl-historian is valuable for two reasons:
...
The database user `uupl-historian` and the SSH account `hist_admin` share the same
...
| SQLi in industrial uupl-historian     | CVE-2021-27663                     | Wonderware Historian: unauthenticated SQL injection                               |
...
The uupl-historian is the information hub of the operational zone. It connects the
...
uupl-historian does not directly affect the physical process, but it reveals everything
needed to do so precisely.

```
discover uupl-historian via bursar-desk ops-access.conf
        ↓
