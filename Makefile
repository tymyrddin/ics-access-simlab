...
.PHONY: help generate up down clean purge test test-unit test-artifacts test-smoke lint
...
help:
	...
	@echo "  make lint           Dead-code gate (vulture) over orchestrator/"
	@echo "  make test           Lint + unit + artifact tests (smoke needs a running lab)"
	@echo ""
...
lint:
	vulture orchestrator/ --min-confidence 60

test: lint test-unit test-artifacts
